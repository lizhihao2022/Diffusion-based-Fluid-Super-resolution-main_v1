            ...
            for repeat in range(self.args.repeat_run):
                ...
                for it in range(self.args.sample_step):  # we run the sampling for three times
                    ...
                    if self.config.sampling.log_loss:
                        ...
                        logger.reset()
            restored_img = scaler.inverse(x)
            # print(restored_img.shape, gt.shape, x.shape)
            restored_img_filename = "restored_img.png"
            path_to_dump = os.path.join(self.image_sample_dir, sample_folder, restored_img_filename)
            make_image_grid(slice2sequence(restored_img), path_to_dump)
